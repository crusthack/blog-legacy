...
- 앞으로의 게임 개발 학습 방향성에 대해서는 유니티에 관해서는 UI 구현 및 2D 게임 제작 정도로만 학습하고 언리얼엔진을 통해 잘 설계된 구조를 학습하고자 합니다. 


**현재 메시지 소비가 매우 수동적인데, 하나의 세션에서 메시지에 번호를 붙이고 해당 번호에 대한 응답을 기다리는 식으로, 다른 웹 통신 라이브러리에서 보았던 것처럼 비슷하게 만들어보는게 좋을 것 같음. 타임아웃 필수**
