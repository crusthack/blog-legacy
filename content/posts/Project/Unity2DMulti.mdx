...
- 네트워크 라이브러리 불안정성. .net standard 2.0으로 포팅되면서 AI가 코드를 어떻게 변경하였는지 검수하지 않았고 그로인해선지 후반부에 알 수 없는 문제가 여럿 발생함.
 유니티 이해도 부족으로 인한 구현 실수도 영향이 있을 것 같음. 에디터에선 잘 되는데 빌드 결과물에선 동작이 이상한 경우가 생김.
 
### 블로그 페이지에 있는 코드는 대부분 제가 직접 작성한 코드입니다. 
- 현재 깃허브 저장소에 있는 라이브러리측 코드는 .Net 10.0으로 작성된 코드를 유니티에서 돌리기 위해 코파일럿으로 .net standard 2.0으로 포팅한 코드입니다만  
블로그 페이지에 있는 코드는 제가 직접 작성하였습니다. 직렬화 부분과 span 을 사용하는 부분은 AI의 도움을 받았습니다. 