...
Players 딕셔너리(Disctionary\<int, Player>)에 추가합니다. (게임 참여 처리)
...
#### 게임중
- 클라이언트 유저는 1초마다 자신의 게임 상태를 호스트 유저에게 보냅니다. 
- 호스트는 해당 SyncMessage를 받은 뒤 처리를 하고, 해당 호스트도 1초마다 전체 플레이어의 상태를 모든 접속 유저들에게 전파합니다. 
...
- 앞으로의 게임 학습 방향성에 대해서는 유니티에 관해서는 UI 구현 및 2D 게임 제작 정도로만 학습하고 언리얼엔진을 통해 잘 설계된 구조를 학습하고자 합니다. 