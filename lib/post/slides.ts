...
export type ElementType = 'simple' | 'complex' | 'html' | 'image' | 'code' | 'math' | 'table' | 'grid';
...
const WEIGHT_GRID_PER_COL = 3;

/** ![ ... ]! 원본 소스에서 최대 열 수를 추정한다. */
function estimateGridCols(gridLines: string[]): number {
  let maxCols = 1;
  let rowCols = 1;
  ...
  for (const line of gridLines) {
    const t = line.trim();
    if (t === '![' || t === ']!') continue;
    if (t.startsWith('```')) { inCodeFence = !inCodeFence; continue; }
    if (inCodeFence) continue;
    if (!inStarBlock && t.startsWith('***') && t.length > 3) { inStarBlock = true; continue; }
    if (inStarBlock && t === '***') { inStarBlock = false; continue; }
    ...
    if (t === ',') {
      ...
      if (pipes > 0) rowCols = Math.max(rowCols, pipes + 1);
    }
  }
...
export function splitContentIntoSlides(content: string): Slide[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 그리드 블럭 ![ ... ]! — 전체를 단일 요소로 수집
    if (!isInCodeBlock && !isInMathBlock && !isInHtmlBlock && line.trim() === '![') {
      const gridLines: string[] = [line];
      let innerCode = false;
      let depth = 1;
      while (depth > 0 && i + 1 < lines.length) {
        ...
        const gt = gl.trim();
        gridLines.push(gl);
        if (gt.startsWith('```')) innerCode = !innerCode;
        if (!innerCode) {
          if (gt === '![') depth++;
          else if (gt === ']!') depth--;
        }
      }
      const gridWeight = Math.min(MAX_WEIGHT_PER_SLIDE, estimateGridCols(gridLines) * WEIGHT_GRID_PER_COL);
      allElements.push({ type: 'grid', content: gridLines.join('\n'), lines: gridLines.length, weight: gridWeight });
      continue;
    }
...
function splitByManualSlideSeparator(content: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (!isInCodeBlock && !isInMathBlock) {
      if (trimmed === '![') gridDepth++;
      else if (trimmed === ']!' && gridDepth > 0) gridDepth--;
    }

    if (!isInCodeBlock && !isInMathBlock && gridDepth === 0 && trimmed === '---') {
...
function analyzeManualSlideContent(content: string): ContentElement[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!isInCodeBlock && !isInMathBlock && line.trim() === '![') {
      ...
      elements.push({ type: 'grid', content: gridLines.join('\n'), lines: gridLines.length, weight: WEIGHT_HTML });
      continue;
    }
